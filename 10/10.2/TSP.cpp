...
int main(int argc, char *argv[]){
    ...
    for(int i=0; i<ngens; i++){
        NewGeneration(pc,pm);
        if((i+1)%100==0) cout << "Rank " << Rank << ": Generation number " << i+1 << " - min length: " << population[0][ncities] << endl << endl;
        if((i+1)%20==0) Migration();
        PrintBestLength(i+1);
    }
...
void Input(void){
    ...
    if(config=="circle") Circumference();
    else if(config=="square") Square();

    if(Rank==0) PrintCities();
    ...
    if(Rank==0){
         ...
         cout << "Genetic Algorithm - Parallel computing"<< endl << endl;
         cout << "The programs works on "<< Size << " nodes" << endl;
         cout << "The program evaluates the shortest path" << endl;
         cout << "  among " << ncities << " cities" << endl;
         if(config=="circle") cout << "  on a circumference with radius=1" << endl << endl;
         else if(config=="square") cout << "  inside a square with side=1" << endl << endl;
...
void PrintBestPath(){
    ...
    ofstream BestPath;
    BestPath.open(to_string(Rank)+"_best_path.out");
...
void PrintBestLength(int istep){
    ...
    ofstream BestLength;
    BestLength.open(to_string(Rank)+"_best_length.out", ios::app);
...
void Migration(void){
    ...
    double migrant[ncities+1];
    for(int i=0; i<=ncities; i++) migrant[i] = population[0][i]; //Best path of the population

    double arrival[ncities+1];

    int cont=-1, cont1=-1, cont2=-1;

    //Extract a random continent
    if(Rank==0){
        cont1=0;
        do{ cont=int(rnd.Rannyu()*Size);
		} while(cont==cont1);
        cont2=cont; //Rank0 paired with Rank==cont
    }

    MPI_Bcast(&cont, 1, MPI_INTEGER, 0, MPI_COMM_WORLD); //Communicate the random cont to all Ranks

    if(Rank==cont){
        cont1=0;
		cont2=cont; //Rank==cont paired with Rank0
    }

    //Pair the remaining two Ranks
    if(Rank!=cont and Rank!=0){
        if(cont==1){
            cont1=2;
            cont2=3;
        }
    ...
    //Sending and receiving the best path beween couples of Ranks
	if(Rank==cont1){
			MPI_Send(&migrant, ncities+1, MPI_DOUBLE, cont2, itag, MPI_COMM_WORLD);
			MPI_Recv(&arrival, ncities+1, MPI_DOUBLE, cont2, itag2, MPI_COMM_WORLD, &stat1);
	}
	else if(Rank==cont2){
			MPI_Send(&migrant, ncities+1, MPI_DOUBLE, cont1, itag2, MPI_COMM_WORLD);
			MPI_Recv(&arrival, ncities+1, MPI_DOUBLE, cont1, itag, MPI_COMM_WORLD, &stat2);
	}

    //Set as first population's path the one received
    for(int j=0; j<=ncities; j++) population[0][j] = arrival[j];

    //The path received is not necessarily the shortest of the Rank's population
    Sort();
}
