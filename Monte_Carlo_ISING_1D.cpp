...
//initial configuration
  if(restart)
  {
    cout << "Restarting the simulation from the spin configuration config.final" <<endl;
    
    ifstream ConfOld("config.final");
    if(!ConfOld.is_open()) cerr << "PROBLEM: config.final does not exist. You may need to run one first time the simulation." << endl;
...
{
  double neq=50*nstep;
  for(int i=0; i<neq; i++) Move(metro);
}
