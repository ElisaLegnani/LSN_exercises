...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Pres.open("output.press.0",ios::app);
...
//g(r)
    for(int b=0; b<nbins; ++b){
      ...
      stima_gofr=blk_av[i]/blk_norm/(rho*npart*deltaV);
      ...
      if(iblk==nblk){
        Gave << setw(wd) << iblk <<  setw(wd) << r << setw(wd) << stima_gofr << setw(wd) << glob_av[i]/(double)iblk << setw(wd) << err_gofr << endl;
      }
    }
